    ...
    try {
        ...
        if (owningCard) {
            ...
            if (cmp(req.userData, owningCard.indirectOwner)) {
                ...
                owningCard.checklists.push(newChecklist._id);
...
export const deleteChecklistByChecklistId: EMiddleware = async (req, res, next) => {
    ...
    try {
        ...
        if (!foundChecklist) {
            ...
            await Card.findByIdAndUpdate(foundChecklist.owner, { $pull: { [CollectionName.Checklist]: foundChecklist._id }, updatedOn }, { session });
